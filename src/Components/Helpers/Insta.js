...
// ref: https://blog.devgenius.io/how-add-your-instagram-posts-to-your-react-project-e8527d2a7187
function InstaFeed () {
    const [instaItems, setInstaItems] = useState([]);

    // TODO: name_not_resolved, need to test locally with some IP address
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            if(!userId || !accessToken) {
                console.log("userID or access token undefined: ", {userId, accessToken});
                return;
            }
        ...
        <div>
            {
                instaItems.map(item => {
                    return(
                        <img src={item.mediaUrl} width="100px" alt="test pic"/>
                    );
                })
            }
        </div>
...
function InstaPic ({ dataInstaPermalink, linkAHref_1, linkAHref_2 }) {
    return (
        <blockquote className="instagram-media" data-instgrm-permalink={dataInstaPermalink}
        data-instgrm-version="14"
        style={{background: "#ffffff", border: 0, borderRadius: "3px", boxShadow: "0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15)",
...
export { InstaFeed, InstaPost, InstaPic, InstaPostCaption };